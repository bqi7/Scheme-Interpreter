...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError ("Argument and formal parameter length mismatch!")
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Error. Too many arguments!")
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(new_target, LambdaProcedure(target.second, expressions.second, env))
        return new_target
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        symbol_pairs = Pair(bindings.first.first, symbol_pairs)
        value_pairs = Pair(scheme_eval(bindings.first.second.first, env), value_pairs)
        bindings = bindings.second
    check_formals(symbol_pairs)
    return env.make_child_frame(symbol_pairs, value_pairs)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        new_target = target.first
        macro_procedure = MacroProcedure(target.second, expressions.second, env)
        env.define(new_target, macro_procedure)
        return new_target
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
